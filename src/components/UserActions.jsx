      ...
      {currentUser?.loggedIn ? (
        <>
          ...
          <button
            ...
            className="bg-red-700 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-full"
          >
            Logout
          </button>
        </>
        ...
        <div className="relative group">
          ...
          <div className="absolute top-full left-0 mt-2 w-48 bg-white border rounded-lg shadow-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-20">
            <Link to="/login" className="block px-4 py-2 text-sm hover:bg-stone-100 uppercase">Login</Link>
            <Link to="/signup" className="block px-4 py-2 text-sm hover:bg-stone-100 uppercase">Sign Up</Link>
          </div>
        </div>
      )}
